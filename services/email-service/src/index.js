...
app.listen(port, () => {
  console.log(`[email-service] Status API running on port ${port}`);
});
...
async function start() {
  ...
  console.log('[email-service] Waiting for user_created events...');
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      console.log(`[email-service] Sending welcome email to ${payload.email}`);
    },
  });
}
...
async function bootstrap() {
  ...
  while (true) {
    try {
      ...
      console.error('[email-service] Kafka connection failed:', error.message);
